...
class Post(models.Model):
    ...
    excerpt = models.TextField(
        null=True, verbose_name='خلاصه مطلب', max_length=253)
    content = RichTextField(config_name='full')
    category = models.ForeignKey(
        Category, on_delete=models.PROTECT, default=1, verbose_name='دسته بندی', null=True)
    date = models.DateTimeField(verbose_name='تاریخ انتشار')
    autor = models.ForeignKey(
        User, on_delete=models.PROTECT, verbose_name='نویسنده')
    status = models.CharField(
        max_length=10, choices=options, default='published', verbose_name='وضعیت')
...
class PostImage(models.Model):
    ...
    image = models.ImageField(upload_to='PostImage',
                              blank=True, verbose_name='تصاویر')
    post = models.ForeignKey(Post, on_delete=models.PROTECT, default=1)
    desc = models.CharField(
        max_length=1000, verbose_name='توضیحات', null=True, blank=True)
...
class GalleryImage(models.Model):
    ...
    desc = models.CharField(
        max_length=200, verbose_name='عکس نوشت', null=True, blank=True)


class Video(models.Model):

    video = models.FileField(
        upload_to='videos', verbose_name='ویدیو', blank=True)
    post = models.ForeignKey(Post, on_delete=models.PROTECT, default=1)
